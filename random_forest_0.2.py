...
class Random_Forest(Decision_Tree):
    ...
    def plant_tree(self, train_data, features, Threshold):
        list_for_features = []
        list_for_features = features.keys()

        for i in range(self.num_1):
            list_for_train_data = self.random_choice_train_data(train_data)
            train_data_temp = []

            for cnt in range(len(list_for_processed)):
                train_data_temp.append(train_data[list_for_processed[cnt]])
            ...
            for j in range(self.num_2):
                ...
                for cnt in range(len(list_for_features)):
                    feature_temp = list_for_features[list_for_features[cnt]]
                    num_temp = features[feature_temp]
                    features_temp.update( {feature_temp:num_temp})

                decision_tree_temp = Decision_Tree(Threshold)
                decision_tree_temp.build_tree(
                    train_data_temp, features_temp, decision_tree_temp.Tree)

                Random_Forest.Forest.append(decision_tree_temp)
